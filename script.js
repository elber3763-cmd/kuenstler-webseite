...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (cursorDot && cursorOutline) {
        document.addEventListener('mousemove', (e) => {
            ...
            // Kreis folgt sanft (GSAP oder Native)
    ...
    function startGalleryCenterShow(allImages) {
        ...
        function playChunk() {
            ...
            currentImages.forEach(werk => {
                ...
                // KORREKTUR: Jetzt mit alt-Attribut für Bilder
                el.innerHTML = `<img src="${werk.bild}" alt="${werk.titel || 'Galerie Bild'}" class="w-full h-full object-cover rounded-lg shadow-lg">`;
            ...
            createdElements.forEach(el => {
                tl.to(el, { zIndex: 100, scale: 1.1, boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.9)", duration: 1.5, ease: "power2.inOut" });
                tl.to({}, { duration: 1.0 }); 
                tl.to(el, { zIndex: 1, scale: 1, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1)", duration: 1.0, ease: "power2.inOut" });
            });
